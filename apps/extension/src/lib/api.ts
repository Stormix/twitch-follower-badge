...
export const getApi = async () => {
  ...
  api.interceptors.response.use(
    ...
    async (error) => {
      ...
      if (error.response && error.response.status === 401 && !originalRequest._retry && originalRequest.url !== '/user/refresh') {
        ...
        try {
          ...
          logger.info('Token refreshed, retrying request to', originalRequest.url);
